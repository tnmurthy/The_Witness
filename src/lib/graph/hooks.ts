...
export const graphKeys = {
  ...
  entities: (type?: string, search?: string) =>
    [...graphKeys.all, "entities", type ?? "all", search ?? ""] as const,
  neighbors: (type: string, id: string, depth: number) =>
    [...graphKeys.all, "neighbors", type, id, depth] as const,
...
export function useGraphNeighbors(type: string, id: string, depth: number) {
  return useQuery({
    ...
    queryFn: () =>
      apiGet<{ neighbors: GraphNeighbor[] }>(`/api/graph/nodes/${type}/${id}/neighbors?depth=${depth}`),
    enabled: !!type && !!id,
  });
}
